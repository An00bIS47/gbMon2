...
void glcd::DrawArray(buffer data, uint8_t color){
    ...
    for(j = 0; j < height / 8; j++) {
        ...
        for(i = 0; i < width; i++) {
            ...
            /*
            bits[counter] = (char)(((int)'0')+data[counter]);


            counter++;
            if (counter==8) {
                
                Serial.println((int) strtol(bits, NULL, 2), DEC);
                counter=0;
            }
            uint8_t displayData = (int) strtol(bits, NULL, 2);
            */
            uint8_t displayData = data[counter];
            counter++;
            if(color == BLACK)
